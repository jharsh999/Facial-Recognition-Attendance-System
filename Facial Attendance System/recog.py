...
class recog(object):
    def Attendance(a):
        ...
        set_students = set(known_face_names)
        students = (list(set_students))
        now = datetime.now()
        current_date = now.strftime("%Y-%m-%d")

        f1 = open(current_date+'.csv','w+',newline = '')
        lnwriter = csv.writer(f1)
                ...
                for face_encoding in face_encodings:
                    ...
                    if name in known_face_names:
                        if name in students:
                            ...
                            print(students)
                            print(name + "_" + ref_dictt[name] + " 's attendance has been done successfully")
                            current_time = now.strftime("%H-%M-%S")
                            lnwriter.writerow([name,ref_dictt[name],current_time,current_date])
